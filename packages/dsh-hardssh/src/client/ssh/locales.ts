...
export const zh = {
  ...
  'form.keyPath': '私钥路径',
  'form.keyPathHint': '私钥文件的绝对路径（如 /home/you/keys/your-key）',
  ...
  'form.tagsHint': '逗号分隔',
  'form.location': '位置',
  'form.commandPolicy': '禁止命令（每行一个正则）',
  'form.commandPolicyHint': '每行一个正则，命中即拦截；以 # 开头的行与空行会被忽略。留空 = 不拦截。',
  'form.denyCommands': '禁止命令名称（每行一个，自动解包）',
  'form.denyCommandsHint': '命令名匹配会自动剥掉包装：`bash -c "python …"`、`sudo python …`、`/usr/bin/python3`、`FOO=1 python …` 都会解析成真实命令名（如 python）再比对。',
  'form.allowCommands': '豁免命令名称（可选，每行一个）',
  'form.allowCommandsHint': '在禁止清单内的例外（例如你允许偶尔内联运行的命令名）。',
  'form.policyHint': '提示信息（可选）',
  'form.policyHintHint': '拦截时显示的提示，例如「请用 srun/sbatch 提交到计算节点」。留空使用默认文案。',
  'form.save': '保存',
  'form.cancel': '取消',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  'form.keyPathHint': 'Absolute path to the private key file (e.g. /home/you/keys/your-key)',
  ...
  'form.commandPolicy': 'Forbidden commands (one regex per line)',
  'form.commandPolicyHint': 'Matching commands are blocked; lines starting with # and blank lines are ignored. Leave empty = no interception.',
  'form.denyCommands': 'Forbidden command names (one per line, auto-unwrapped)',
  'form.denyCommandsHint': 'Names are matched after unwrapping: `bash -c "python …"`, `sudo python …`, `/usr/bin/python3`, `FOO=1 python …` all resolve to the real command name (e.g. python) before matching.',
  'form.allowCommands': 'Allowed command names (optional, one per line)',
  'form.allowCommandsHint': 'Exceptions within the denied list (names you allow to run inline).',
  'form.policyHint': 'Message shown on block (optional)',
  'form.policyHintHint': 'For example: "Use srun/sbatch to submit to a compute node". Left empty uses the default text.',
  'form.save': 'Save',
  'form.cancel': 'Cancel',
